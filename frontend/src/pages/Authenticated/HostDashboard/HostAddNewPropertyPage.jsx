...
const AmenityItem = styled.div`
  ...
  background-color: ${({ isSelected }) =>
    isSelected ? "rgba(173, 216, 230, 0.7)" : "transparent"};
  width: 248px;
  height: 163px;

  &:hover {
    background-color: ${({ isSelected }) =>
      isSelected ? "rgba(173, 216, 230, 0.7)" : "rgba(173, 216, 230, 0.3)"};
  }

  span {
    background-color: ${({ isSelected }) =>
      isSelected ? "rgba(173, 216, 230, 0.3)" : "transparent"};

    &:focus {
      outline: none;
      background-color: ${({ isSelected }) =>
        isSelected ? "rgba(173, 216, 230, 0.7)" : "rgba(173, 216, 230, 0.7)"};
    }
  }
`;
...
const HostAddNewPropertyPage = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const propertyFormData = useSelector(
    (state) => state.general.propertyFormData
  );
  ...
  useEffect(() => {
    dispatch(
      setPropertyForm({ field: "amenities_ids", value: selectedAmenities })
    );
  ...
  const handleSubmit = async (event) => {
    ...
    fieldsToSubmit.forEach((field) => {
      const value = propertyFormData[field];
    ...
    // Append amenities
    selectedAmenities.forEach((id) => formData.append("amenities_ids", id));

    // Append images
    console.log("Selected images:", selectedImages);
    selectedImages.forEach((imageFile) => {
      console.log("Appending image:", imageFile);
      formData.append("upload_images", imageFile); // or 'images[]'
    });
    ...
    try {
      ...
      toast.success("A new property was successfully added");

      setTimeout(() => {
        navigate("/host-properties");
        dispatch(clearPropertyForm()); // clear inputs
      }, 3000);
    } catch (error) {
  ...
  return (
    <HostDashboardLayout>
      <Container>
        {currentSlide === 1 && (
          <>
            <SlideLabel>Add a new property</SlideLabel>
...
export default HostAddNewPropertyPage;
